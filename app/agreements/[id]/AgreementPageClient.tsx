...
export default function AgreementPageClient({ agreement, data }: AgreementPageProps) {
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    if (!loaded) { alert('Failed to load payment gateway. Please refresh and try again.'); setIsSubmitting(false); return; }
    ...
    try {
      const res = await fetch('/api/razorpay-order', {
        ...
        body: JSON.stringify({
          ...
          notes: { product: agreement.name, href: agreement.href, email: formData.email },
        }),
      });
            ...
            <form onSubmit={handleFormSubmit} noValidate>
              ...
              <div className="flex gap-3">
                <button type="button" onClick={() => setShowForm(false)}
                  className="flex-1 border border-gray-300 text-gray-700 px-4 py-2 rounded-md text-sm hover:bg-gray-50 transition">Cancel</button>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
              ...
              <div className="border border-gray-200 rounded-lg bg-gray-50 overflow-hidden" style={{ userSelect: "none" }}>
                ...
                <div className="max-h-[720px] overflow-y-auto p-3 space-y-3" onContextMenu={(e) => e.preventDefault()}>
                  {agreement.previewImages.map((src: string, i: number) => (
                    <img key={src} src={src} alt={agreement?.name + ' page ' + (i+1)} draggable={false}
            ...
            <div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-5 mb-6">
                <p className="text-2xl font-semibold text-gray-900 mb-3">Rs.{agreement?.price?.amount?.toLocaleString("en-IN")}</p>
                <button onClick={handleGetDraftClick} className="inline-block bg-black text-white px-6 py-2 rounded-md hover:bg-gray-800 transition cursor-pointer">Get Draft @ Rs.{agreement?.price?.amount}</button>
              </div>
              <p className="text-gray-700 text-sm leading-relaxed mb-6">{agreement?.longDescription}</p>
              {agreement?.keyFeatures?.length > 0 && (<section className="mb-8"><h3 className="text-lg font-semibold text-gray-900 mb-3">Key Features</h3><ul className="list-disc pl-5 space-y-1 text-gray-700">{agreement.keyFeatures.map((item, index) => (<li key={index}>{item}</li>))}</ul></section>)}
              {agreement?.idealFor?.length > 0 && (<p className="mb-4 text-gray-700"><strong className="text-gray-900">Ideal for: </strong>{agreement.idealFor.join(", ")}</p>)}
              {agreement?.Note && (<div className="mb-6"><p className="text-gray-700"><strong className="text-gray-900">Note: </strong>{agreement.Note}</p></div>)}
              {agreement?.keyDifferentiators?.length > 0 && (<p className="mb-4 text-gray-700"><strong className="text-gray-900">Key Differentiator: </strong>{agreement.keyDifferentiators.join(", ")}</p>)}
              {agreement?.tableOfContents?.length > 0 && (<><h3 className="text-lg font-semibold text-gray-900 mb-2">Table of Contents</h3><ul className="list-disc pl-6 text-gray-700">{agreement.tableOfContents.map((item, i) => (<li key={i}>{item}</li>))}</ul></>)}
              {agreement?.disclaimer && (<p className="mt-6 text-gray-700 text-sm border-t border-gray-200 pt-4 font-semibold"><strong className="text-gray-900">Disclaimer:</strong> {agreement.disclaimer}</p>)}
            </div>
          </div>
        </div>
      </div>
