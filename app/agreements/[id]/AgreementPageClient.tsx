...
export default function AgreementPageClient({ agreement, data }: AgreementPageProps) {
  ...
  const handleGetDraftClick = async (e: React.MouseEvent<HTMLButtonElement>) => {
    e.preventDefault();
            ...
            <div>
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-5 mb-6">
                ...
                <button
                  ...
                  className="inline-block bg-black text-white px-6 py-2 rounded-md hover:bg-gray-800 transition cursor-pointer disabled:opacity-50"
                >
                  {isSubmitting ? 'Please wait...' : `Get Draft @ ₹${agreement?.price?.amount}`}
                </button>
              </div>
              ...
              {agreement?.Note && (
                <div className="mb-6">
                  <p className="text-gray-700"><strong className="text-gray-900">Note: </strong>{agreement.Note}</p>
                </div>
              )}
